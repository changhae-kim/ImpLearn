...
class Gaussian():
    ...
    def __init__(self, file_paths, prefixes,
        ...
        if isinstance(cp_mults, str):
            self.cp_mults = [cp_mults] * n_struct
        else:
            self.cp_mults = cp_mults

        if isinstance(cp_frags, int):
            self.cp_frags = [[cp_frags]] * n_struct
        elif cp_frags == [] or isinstance(cp_frags[0], int):
            self.cp_frags = [cp_frags] * n_struct
        else:
            self.cp_frags = cp_frags
        ...
        for i in range(n_struct):
            ...
            for j, _ in enumerate(self.structures[i]):
                ...
                if self.cp_frags[i] != []:
                    ...
                    state, optimizer, cluster = i, optimizer, self.clusters[i][j]

                    atoms = cluster.get_chemical_symbols()
                    coords = cluster.get_positions()

                    header = '''%NProcShared={n_proc:d}
#n {method:s} NoSymm SCF=XQC Counterpoise={n_frags:d}

 {label:s}

 {charge:d} {mult:d} {cp_mult:s}
'''.format(n_proc=self.n_proc, method=self.method, n_frags=max(self.cp_frags[state]), label=os.path.basename(optimizer), charge=self.charge[state], mult=self.mults[state], cp_mults=self.cp_mults[state])
                    body = ''
                    for k, (X, coord) in enumerate(zip(atoms, coords)):
                        if k in self.frozen_atoms[state]:
                            atom_type = -1
                        else:
                            atom_type = 0
                        atom_frag = '{:s}(Fragment={:d})'.format(X, self.cp_frags[state][k])
                        body += '{X:14s} {t:2d} {x:9f} {y:9f} {z:9f}\n'.format(X=atom_frag, t=atom_type, x=coord[0], y=coord[1], z=coord[2])
                    footer = '\n'
...
'''.format(n_proc=self.n_proc, method=self.method, opt=opt, freq=freq, label=os.path.basename(optimizer), charge=self.charges[state], mult=self.mults[state])
    ...
    def add_conformers(self, prefixes, degeneracies=1, cp_prefixes=None):
